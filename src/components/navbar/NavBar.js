...
//           ) : (
//             <FaTimes className="icon-close" onClick={handleNav} />
//           )}
...
const Navbar = () => {
  const [nav, setNav] = useState(false);
  const handleNav = () => setNav(!nav);
  const location = useLocation(); // Hook для получения текущего пути
  ...
  // Закрытие сайдбара при переходе на другую страницу
  const closeNav = () => {
    setNav(false);
  };
